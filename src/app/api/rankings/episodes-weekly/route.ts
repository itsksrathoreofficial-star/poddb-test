...
export async function GET(request: NextRequest) {
  try {
    ...
    // Try to get weekly data from episode_daily_stats first
    ...
    if (!dailyStatsError && dailyStatsData && dailyStatsData.length > 0) {
      ...
      // Calculate weekly gains for each episode
      const weeklyRankings = Array.from(episodeMap.values()).map(episode => {
        ...
        const weeklyWatchTime = Math.max(0, (lastDay.watch_time || 0) - (firstDay.watch_time || 0));
    ...
    const weekAgoFallback = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString();
    ...
    const { data: episodesData, error: episodesError } = await supabase
      ...
      .select(`
        *,
        podcasts!inner(id, title, slug)
      `)
      .gte('created_at', weekAgoFallback)
      .order('views', { ascending: false });

    if (episodesError) {
      console.error('Error fetching episodes weekly data:', episodesError);
      return NextResponse.json({ error: 'Failed to fetch weekly data' }, { status: 500 });
    }

    // Process episodes data for weekly rankings (fallback)
    // For fallback, we'll use episodes table data and use total views as weekly views
    const rankings = episodesData.map((episode: any, index: number) => {
      return {
        ...
        weekly_views: episode.views || 0, // Use total views as weekly views in fallback
    ...
    console.error('Error in episodes weekly rankings:', error);
