...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!dailyStatsError && dailyStatsData && dailyStatsData.length > 0) {
      ...
      // Group all data by podcast and calculate monthly totals
      data.forEach((item: any) => {
        ...
        if (!podcastMap.has(podcastId)) {
          podcastMap.set(podcastId, {
            ...
            monthly_views: 0, // Sum of all daily views in month
            monthly_likes: 0,
            monthly_comments: 0,
        ...
        // Add to monthly totals (sum of all daily data in the month)
        podcast.monthly_views += item.views || 0;
        podcast.monthly_likes += item.likes || 0;
        podcast.monthly_comments += item.comments || 0;
        podcast.monthly_watch_time += item.total_watch_time || 0;
        podcast.monthly_episodes += item.new_episodes || 0;
    ...
    if (podcastsError) {
      console.error('Error fetching podcasts monthly data:', podcastsError);
      return NextResponse.json({ error: 'Failed to fetch monthly data' }, { status: 500 });
    }

    // Process podcasts data for monthly rankings (fallback)
    const rankings = podcastsData.map((podcast: any, index: number) => ({
      ...
      daily_watch_time_gain: 0,
      new_episodes_count: 0,
      monthly_views: podcast.views || 0, // Use total views as monthly views in fallback
      monthly_likes: podcast.likes || 0,
      monthly_comments: podcast.comments || 0,
      monthly_watch_time: podcast.total_watch_time || 0,
      monthly_episodes: 0,
    ...
    console.error('Error in monthly rankings API:', error);
